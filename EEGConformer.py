...
class ClassificationHead(nn.Sequential):
    def __init__(self, emb_size, n_classes, subjects):
        super().__init__()
        ...
        self.fc_subjects = nn.Sequential(
            nn.Linear(2440, 256),
            nn.ELU(),
            nn.Dropout(0.5),
            nn.Linear(256, 32),
            nn.ELU(),
            nn.Dropout(0.3),
            nn.Linear(32, subjects)
        )

    def forward(self, x):
        x = x.contiguous().view(x.size(0), -1)
        out_task = self.fc_tasks(x)
        out_subject = self.fc_subjects(x)
...
class EEGConformer(nn.Sequential):
    def __init__(self, emb_size=40, depth=10, num_classes=2, model_name_prefix='EEGConformer', samples=1001, channels=3, subjects=9):
        if num_classes==2:
            depth=10
        else:
            depth=6
        super().__init__(
            ...
            ClassificationHead(emb_size, num_classes, subjects)
        )
