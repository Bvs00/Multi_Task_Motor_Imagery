...
if [ -z "$NET" ] || [ -z "$PRIME" ] || [ -z "$AUG" ] || [ -z "$SAVED_PATH" ] || [ -z "$NORM" ] || [ -z "$BANDPASS" ] \
...
for seed in "${primes[@]}"; do
    ...
    python -u train_motor_imagery.py --seed "$seed" --name_model "$network" --saved_path "${saved_path}_Wout_Aug" --lr 0.001 \
            --augmentation "None" --num_augmentations 0 --num_workers 5 --normalization "$normalization" \
            ...
            --p1 "$p1" --p2 "$p2" --reduction 2
    echo "Test seed: $seed"
    python -u test_motor_imagery.py --name_model "$network" --saved_path "${saved_path}_Wout_Aug" --paradigm "$paradigm" \
...
python create_excel_motor_imagery.py --network "$network" --path "${saved_path}_Wout_Aug"

for num_aug in "${num_augs[@]}"; do
    for seed in "${primes[@]}"; do
    echo "Train seed: $seed"
    python -u train_motor_imagery.py --seed "$seed" --name_model "$network" --saved_path "${saved_path}_Aug_${num_aug}" --lr 0.001 \
            --augmentation "$aug" --num_augmentations "$num_aug" --num_workers 5 --normalization "$normalization" \
            --paradigm "$paradigm" --train_set "/mnt/datasets/eeg/Dataset_BCI_${dataset}/Signals_BCI_2classes/train_${dataset}_$bandpass.npz" \
            --alpha "$alpha" --patience 150 --batch_size 72 --auxiliary_branch "$aux" --feature_maps $maps \
            --p1 "$p1" --p2 "$p2" --reduction 2
    echo "Test seed: $seed"
    python -u test_motor_imagery.py --name_model "$network" --saved_path "${saved_path}_Aug_${num_aug}" --paradigm "$paradigm" \
            --test_set "/mnt/datasets/eeg/Dataset_BCI_${dataset}/Signals_BCI_2classes/test_${dataset}_$bandpass.npz" \
            --seed "$seed" --alpha "$alpha" --auxiliary_branch "$aux" --feature_maps $maps \
            --p1 "$p1" --p2 "$p2" --reduction 2
    done
    python create_excel_motor_imagery.py --network "$network" --path "${saved_path}_Aug_${num_aug}"
done
