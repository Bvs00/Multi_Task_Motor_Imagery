...
echo "Reduction: $reduction"
    ...
    python -u train_motor_imagery.py --seed "$seed" --name_model "$network" --saved_path "${saved_path}_Reduction_${reduction}" --lr 0.001 \
            --augmentation "$aug" --num_augmentations 3 --num_workers 5 --normalization "$normalization" \
            ...
            --p1 "$p1" --p2 "$p2" --reduction "$reduction"
    echo "Test seed: $seed"
    python -u test_motor_imagery.py --name_model "$network" --saved_path "${saved_path}_Reduction_${reduction}" --paradigm "$paradigm" \
    ...
    python create_excel_motor_imagery.py --network "$network" --path "${saved_path}_Reduction_${reduction}"
done
