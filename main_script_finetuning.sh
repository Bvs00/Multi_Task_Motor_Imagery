#!/bin/bash

if [ -z "$NET" ] || [ -z "$PRIME" ] || [ -z "$AUG" ] || [ -z "$SAVED_PATH_LOSO" ] || [ -z "$SAVED_PATH" ] || [ -z "$NORM" ] \
    || [ -z "$BANDPASS" ] || [ -z "$PARADIGM" ] || [ -z "$PROBING" ] || [ -z "$AUX" ] || [ -z "$DATASET" ]; then
    echo "Errore: Devi specificare NET, PRIME, AUG, SAVED_PATH, NORM, BANDPASS, PARADIGM!"
    echo "Utilizzo: NET=<valore> PRIME=<valore> AUG=<valore> ./script.sh"
    exit 1
fi

echo "$NET"
echo "$PRIME"
echo "$AUG"
echo "$SAVED_PATH_LOSO"
echo "$SAVED_PATH"
echo "$NORM"
echo "$BANDPASS"
echo "$PARADIGM"
echo "$PROBING"
echo "$AUX"
echo "$DATASET"

if [ "$PRIME" == "1" ]; then
  primes=(42 71 101 113 127 131 139 149 157 163 173 181 322 521)
elif [ "$PRIME" == "2" ]; then
  primes=(402 701 1001 1013 1207 1031 1339 1449 1527 1613 1743 1841 3222 5421)
elif [ "$PRIME" == "3" ]; then
  primes=(42 71 101 113 127 131 139)
elif [ "$PRIME" == "4" ]; then
  primes=(149 157 163 173 181 322 521)
fi

echo "${primes[@]}"
network="$NET"
aug="$AUG"
saved_path_loso="$SAVED_PATH_LOSO"
saved_path="$SAVED_PATH"
normalization="$NORM"
bandpass="$BANDPASS"
paradigm="$PARADIGM"
probing="$PROBING"
aux="$AUX"
dataset="$DATASET"

for seed in "${primes[@]}"; do
  echo "Train seed: $seed"
  python -u fine_tuning_loso.py --seed "$seed" --name_model "$network" --saved_path_loso "$saved_path_loso" \
          --saved_path "$saved_path" --lr 0.001 --linear_probing "$probing" \
          --augmentation "$aug" --num_workers 5 --normalization "$normalization" \
          --train_set "/cache/sbove/datasets/eeg/Dataset_BCI_${dataset}/Signals_BCI_2classes/train_${dataset}_$bandpass.npz" \
          --patience 100 --batch_size 72 --auxiliary_branch "$aux"
  python -u test_motor_imagery.py --name_model "$network" --saved_path "$saved_path" --paradigm "$paradigm" \
          --test_set "/cache/sbove/datasets/eeg/Dataset_BCI_${dataset}/Signals_BCI_2classes/test_${dataset}_$bandpass.npz" \
          --seed "$seed" --auxiliary_branch "$aux"
done

python create_excel_motor_imagery.py --network "$network" --path "$saved_path"
echo 'ok'