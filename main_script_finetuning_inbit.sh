...
for seed in "${primes[@]}"; do
  ...
  python -u fine_tuning_loso.py --seed "$seed" --name_model "$network" --saved_path_loso "$saved_path_loso" \
          ...
          --train_set "/home/inbit/Scrivania/Datasets/${dataset}/Signals_BCI_2classes/train_${dataset}_$bandpass.npz" \
...
python create_excel_motor_imagery.py --network "$network" --path "$saved_path" -fine_tuning
echo 'ok'