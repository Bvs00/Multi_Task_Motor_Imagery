import sys
from utils import create_tensors, find_minum_loss, validate_autoencoder, plot_confusion_matrix, \
...
if __name__ == '__main__':
    ...
    parser.add_argument("--name_model", type=str, default='PatchEmbeddingNet_Autoencoder', help="Name of model that use", choices=available_network)
    ...
    for patient in range(len(data_test_tensors)):
        ...
        val_loss_reconstruction, val_f1_tasks, 
        val_accuracy_tasks, 
        val_balanced_accuracy_tasks) = validate_autoencoder(model, test_loader, criterion_tasks, criterion_reconstruction, alpha=args.alpha, device=args.device, masked=0, test=True)
        ...
        loss_list.append(val_loss), loss_list_tasks.append(val_loss_tasks), loss_list_reconstruction.append(val_loss_reconstruction)
