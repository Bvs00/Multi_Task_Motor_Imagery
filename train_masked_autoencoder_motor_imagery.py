from utils import train_model_autoencoder, plot_training_complete_autoencoder, normalize_subset_autoencoder, create_tensors, fix_seeds,\
...
def _train(data, labels, saved_path):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
        ...
        train_tensor, val_tensor = normalize_subset_autoencoder(train_subset, val_subset, normalization_factory_methods[args.normalization])
        ...
        train_model_autoencoder(model=model, fold_performance=fold_performance, train_loader=train_loader, val_loader=val_loader, fold=fold, lr=args.lr,
                    class_weight=class_weights, epochs=args.epochs, device=args.device, augmentation=args.augmentation, patience=args.patience, 
                    alpha=args.alpha, masked=args.masked)

    with open(f'{saved_path}/{args.name_model}_seed{args.seed}_validation_log.txt', 'w') as f:
        pass

    plot_training_complete_autoencoder(fold_performance,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--name_model", type=str, default='PatchEmbeddingNet_Autoencoder', help="Name of tensors that use", choices=available_network)
    ...
    parser.add_argument('--masked', type=int, default=400)
